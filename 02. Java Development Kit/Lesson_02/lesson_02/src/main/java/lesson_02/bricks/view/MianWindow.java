package lesson_02.bricks.view;

import lesson_02.bricks.exceptions.BrickOverflowException;
import lesson_02.bricks.sprites.Background;
import lesson_02.bricks.sprites.Brick;
...
public class MianWindow extends JFrame implements CanvasRepaintListener, Thread.UncaughtExceptionHandler {
    ...
    public void addSprite(int x, int y) {
        if (countSprites >= MAX_COUNT_SPRITES) {
            throw new BrickOverflowException();
        }
        interactable[countSprites++] = new Brick(x, y);
    }
    ...
    public void uncaughtException(Thread t, Throwable e) {
        if (e instanceof BrickOverflowException) {
            e.fillInStackTrace();
        }
    }
    ...
    public void iconBricks() {
        ...
        try {
            ImageIcon imageIcon = new ImageIcon("src/images/bricks.png");
