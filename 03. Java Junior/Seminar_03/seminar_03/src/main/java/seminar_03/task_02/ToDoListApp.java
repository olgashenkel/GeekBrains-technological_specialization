...
//                String s = xmlMapper.writeValueAsString(tasks);
        ...
        if (file.exists()) {
            try {
                if (fileName.endsWith(".json")) {
                    ...
                    tasks = xmlMapper.readValue(file, xmlMapper.getTypeFactory().constructCollectionType(List.class,
                            ToDo.class));
                }
    ...
    // Метод для отображения состояния задач ("выполнено" или "не выполнено")
    ...
    // Метод вывода списка задач в консоль
